...
const ASSET_NAME_REGEXP = /^armor-\S+.zip$/;

const GITHUB_OWNER = 'armor';
const GITHUB_REPO = 'armor-build-store';
...
const configure = function configure (gulp, opts) {
  ...
  gulp.task('github:upload', async function githubUpload () {
    ...
    const releaseTag = `armor-build-${buildName}`;
    const releaseFile = `armor-${buildName}.zip`;
    ...
    try {
      ...
      const res = await octokit.repos.createRelease({
        ...
        repo,
        tag_name: releaseTag,
        name: `Armor build ${buildName}`,
        body: `Armor build for commit ${buildName}\n'${commitMessage}'`,
      });
      ...
      const file = path.resolve(root, 'armor.zip');
  ...
  gulp.task('github:download', async function githubDownload () {
    ...
    for (const asset of res.data.assets) {
      if (ASSET_NAME_REGEXP.test(asset.name)) {
        ...
        const writer = fs.createWriteStream(`${tempDir}/armor.zip`);
    ...
    throw new Error(`Unable to find Armor build asset`);
  });
  ...
  gulp.task('saucelabs:upload', async function sauceLabsUpload () {
    ...
    form.append('file', fs.createReadStream(`${tempDir}/armor.zip`));
    const options = {
      method: 'POST',
      url: `https://saucelabs.com/rest/v1/storage/${process.env.SAUCE_USERNAME}/armor.zip?overwrite=true`,
   ...
   *   [1] 38060
   *   [08:56:53] Using gulpfile ~/code/armor-xcuitest-driver/gulpfile.js
